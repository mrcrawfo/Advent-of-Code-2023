// https://jsfiddle.net/mrcrawfo/hwjb7sr3/
...
const findNumbers = (inputArray) => {
	const numbers = [];
  
  let currentNumber = '';
  let startRow, startCol;
	for (let row in inputArray) {
  	for (let col in inputArray[0]) {
      const currentCharacter = inputArray[row].split('')[col];
    	if (parseInt(currentCharacter) >= 0) {
      	...
      	if (currentNumber.length) {
        	numbers.push ({ number: currentNumber, r: startRow, c: startCol });
          currentNumber = '';
        }
      }
    }
  ...
  const checkSymbol = (inputCharacter) => {
  	return !!inputCharacter.match(/[@#$%&*=\-+\/]+/gm);
  };
  
  for (let n in numbers) {
  	let number = numbers[n];
    let isPartNumber = false;
    for (let characterIndex in number.number) {
    	characterIndex = parseInt(characterIndex);
			if (characterIndex === 0) {
      	if (number.c > 0) {
        	if (number.r > 0) {
          	// check up-left
          	isPartNumber = isPartNumber || checkSymbol(inputArray[number.r - 1].split('')[number.c + characterIndex - 1]);
          }
          if (number.r < inputArray.length - 1) {
          	// check down-left
            isPartNumber = isPartNumber || checkSymbol(inputArray[number.r + 1].split('')[number.c + characterIndex - 1]);
          }
          // check left
          isPartNumber = isPartNumber || checkSymbol(inputArray[number.r].split('')[number.c + characterIndex - 1]);
        }
      }
      if (characterIndex === number.number.length - 1) {
      	if (number.c + characterIndex < inputArray[0].length - 1) {
        	if (number.r > 0) {
          	// check up-right
          	isPartNumber = isPartNumber || checkSymbol(inputArray[number.r - 1].split('')[number.c + characterIndex + 1]);
          }
      ...
      if (number.r > 0) {
        // check up
        isPartNumber = isPartNumber || checkSymbol(inputArray[number.r - 1].split('')[number.c + characterIndex]);
      }
      if (number.r < inputArray.length - 1) {
        // check down
        isPartNumber = isPartNumber || checkSymbol(inputArray[number.r + 1].split('')[number.c + characterIndex]);
    ...
    numbers[n].partNumber = isPartNumber;
  }
    
  return numbers;
};
  
const testArray1 = (inputArray) => {
	const numbers = findNumbers(inputArray);
...
console.log(testArray1(input1));
// 4361
console.log('Answer - Part 1 - Input 2');
console.log(testArray1(input2));
// 536202

const findGears = (inputArray) => {
	const gears = [];
  ...
  for (let row in inputArray) {
  	for (let col in inputArray[0]) {
    	row = parseInt(row);
      col = parseInt(col);
      if (inputArray[row].split('')[col] === '*') {
      	gears.push({ r: parseInt(row), c: parseInt(col) });
      }
    }
  }
...
const findPartNumber = (numbers, r, c) => {
	const rowNumbers = numbers.filter((number) => number.r === r);
  for (let number of rowNumbers) {
  	if (c >= number.c && c <= number.c + number.number.length - 1) {
      return number;
    }
  }
  return null;
...
const testArray2 = (inputArray) => {
	const numbers = findNumbers(inputArray);
  
  const gears = findGears(inputArray);
  for (let g in gears) {
  	const gear = gears[g];
  	const adjacentPartNumbers = new Set();
    
    if (gear.c > 0) {
      if (gear.r > 0) {
        // check up-left
        let p1 = findPartNumber(numbers, gear.r - 1, gear.c - 1);
        if (p1) adjacentPartNumbers.add(parseInt(p1.number));
      }
      if (gear.r < inputArray.length - 1) {
        // check down-left
        let p2 = findPartNumber(numbers, gear.r + 1, gear.c - 1);
        if (p2) adjacentPartNumbers.add(parseInt(p2.number));
      }
      // check left
      let p3 = findPartNumber(numbers, gear.r, gear.c - 1);
      if (p3) adjacentPartNumbers.add(parseInt(p3.number));
    }
    if (gear.c < inputArray[0].length - 1) {
      if (gear.r > 0) {
        // check up-right
        let p4 = findPartNumber(numbers, gear.r - 1, gear.c + 1);
        if (p4) adjacentPartNumbers.add(parseInt(p4.number));
      }
      if (gear.r < inputArray.length - 1) {
        // check down-right
        let p5 = findPartNumber(numbers, gear.r + 1, gear.c + 1);
        if (p5) adjacentPartNumbers.add(parseInt(p5.number));
      }
      // check right
      let p6 = findPartNumber(numbers, gear.r, gear.c + 1);
      if (p6) adjacentPartNumbers.add(parseInt(p6.number));
    }
    if (gear.r > 0) {
      // check up
      let p7 = findPartNumber(numbers, gear.r - 1, gear.c);
      if (p7) adjacentPartNumbers.add(parseInt(p7.number));
    }
    if (gear.r < inputArray.length - 1) {
      // check down
      let p8 = findPartNumber(numbers, gear.r + 1, gear.c);
      if (p8) adjacentPartNumbers.add(parseInt(p8.number));
    }
    
    gears[g].adjacentPartNumbers = Array.from(adjacentPartNumbers);
  }
    
  let total = 0;
  for (let gear of gears) {
  	if (gear.adjacentPartNumbers.length === 2) {
    	const product = gear.adjacentPartNumbers[0] * gear.adjacentPartNumbers[1];
    	total += product;
    }
  }
...
console.log(testArray2(input1));
// 467835
console.log('Answer - Part 2 - Input 2');
console.log(testArray2(input2));
// 78272573
