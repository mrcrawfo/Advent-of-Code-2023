// https://jsfiddle.net/mrcrawfo/6wn0h3jq/
...
const parseGame = (inputString) => {
	const gameNumber = parseInt(inputString.split(':')[0].split('Game ')[1]);
  const output = { gameNumber, sets: [] };
  
  const sets = inputString.split(':')[1].split('; ');
  for (let set of sets) {
    let r = set.match(/\d* red/);
    r = r ? parseInt(r[0]) : 0;
    let g = set.match(/\d* green/);
    g = g ? parseInt(g[0]) : 0;
    let b = set.match(/\d* blue/);
    b = b ? parseInt(b[0]) : 0;
  	output.sets.push({ r, g, b });
  }
  
  return output;
};

const isGamePossible = (game, inputSet) => {
	const maxR = maxColor(game.sets, 'r');
	const maxG = maxColor(game.sets, 'g');
	const maxB = maxColor(game.sets, 'b');
  
  return inputSet.r >= maxR && inputSet.g >= maxG && inputSet.b >= maxB;
};

const test1 = (gameArray, inputSet) => {
	let total = 0;
  
  for (let game of gameArray) {
  	const formattedGame = parseGame(game);
  	let result = isGamePossible(formattedGame, inputSet);
    if (result) {
    	total += formattedGame.gameNumber;
    }
  }
...
console.log(test1(input1, { r: 12, g: 13, b: 14 }));
// 8
console.log('Answer - Part 1 - Input 2');
console.log(test1(input2, { r: 12, g: 13, b: 14 }));
// 1867

const test2 = (gameArray) => {
	let total = 0;
  ...
  for (let game of gameArray) {
  	const formattedGame = parseGame(game);
    const maxR = maxColor(formattedGame.sets, 'r');
    const maxG = maxColor(formattedGame.sets, 'g');
    const maxB = maxColor(formattedGame.sets, 'b');
  	let minimalSet = { r: maxR, g: maxG, b: maxB};
    total += findPower(minimalSet);
  }
