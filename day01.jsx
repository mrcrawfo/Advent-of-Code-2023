// https://jsfiddle.net/mrcrawfo/psra5xg8/
...
const getFirstDigit = (inputString) => {
...
const getLastDigit = (inputString) => {
...
const testString = (inputString) => {
	const firstDigit = getFirstDigit(inputString);
  const lastDigit = getLastDigit(inputString);
  
  return parseInt(firstDigit + lastDigit);
};

const testArray = (inputArray) => {
	let total = 0;
...
console.log(testArray(input1));
console.log('Answer - Part 1 - Input 2');
console.log(testArray(input2));
...
const replaceWordsWithNumbers = (inputString) => {    
  const replaceFirst = (inputStr, matches) => {
    let firstIndex = 999;
    let pairIndex = -1;
    for (let i = 0; i < matches.length; i++) {
      if (matches[i] >= 0 && matches[i] < firstIndex) {
        firstIndex = matches[i];
        pairIndex = i;
      }
    }

    return inputStr.substring(0, firstIndex) + wordsToNumbers[pairIndex][1] + inputStr.substring(firstIndex + 1);
  };
  
  let matches = [];
  for (let p = 0; p < wordsToNumbers.length; p++) {
    const pair = wordsToNumbers[p];
    matches.push(inputString.indexOf(pair[0]));
  }
  if (Math.max(...matches) >= 0) {
	  while (Math.max(...matches) >= 0) {
  		inputString = replaceFirst(inputString, matches);
      matches = [];
      for (let p = 0; p < wordsToNumbers.length; p++) {
        const pair = wordsToNumbers[p];
        matches.push(inputString.indexOf(pair[0]));
...
const testCorrectedString = (inputString) => {
  inputString = replaceWordsWithNumbers(inputString);
  
	const firstDigit = getFirstDigit(inputString, true);
  const lastDigit = getLastDigit(inputString, true);
  
  return parseInt(firstDigit + lastDigit);
};

const testCorrectedArray = (inputArray) => {
	let total = 0;
  for (let s of inputArray) {
  	total += testCorrectedString(s);
  }
  return total;
}

console.log('Answer - Part 2 - Input 3');
console.log(testCorrectedArray(input3));
console.log('Answer - Part 2 - Input 2');
console.log(testCorrectedArray(input2));
