...
const processWorkflows = (input) => {
	let workflows = {};
  for (let workflow of input) {
  	let [name, rules] = workflow.split('{');
    rules = rules.slice(0, rules.length - 1).split(',');
    workflows['_' + name] = rules;
  }
  return workflows;
};
...
const test1 = (input) => {
  ...
  let workflows = processWorkflows(input.workflows);
  ...
  const runWorkflow = (part, rules) => {
    for (let rule of rules) {
      ...
      if (rule.indexOf(':') < 0) {
        return runWorkflow(part, workflows['_' + rule]);
      }
      
      let [condition, nextRule] = rule.split(':');
      let partProp = condition.slice(0, 1);
      let operator = condition.slice(1, 2);
      let value = parseInt(condition.slice(2, condition.length));
      let result;
      if (operator === '>') {
      	result = !!(part[partProp] > value);
      }
      if (operator === '<') {
      	result = !!(part[partProp] < value);
      }
      if (result) {
        ...
        return runWorkflow(part, workflows['_' + nextRule]);
      }
    }
  };
  
  for (let part of input.parts) {
  	runWorkflow(part, workflows._in);
  }
  
  console.log(approved);
  
  let total = 0;
  for (let part of approved) {
  	total += part.x;
  	total += part.m;
  	total += part.a;
  	total += part.s;
