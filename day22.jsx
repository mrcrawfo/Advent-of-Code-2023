...
const test1 = (input) => {
    ...
    for (let brick of bricks) {
        ...
        let supportedBricks = bricks.filter((b) => b.name !== brick.name && b.cubes.some((c) => brick.cubes.some((c2) => c2.x === c.x && c2.y === c.y && c2.z + 1 === c.z)));
        // console.log(`Brick ${brick.name} supports ${supportedBricks.length} bricks`);
        for (let supportedBrick of supportedBricks) {
            // console.log(` -- Brick ${brick.name} supports Brick ${supportedBrick.name}`);
            let supportingBricks = bricks.filter((b) => b.name !== supportedBrick.name && b.cubes.some((c) => supportedBrick.cubes.some((c2) => c2.x === c.x && c2.y === c.y && c2.z - 1 === c.z)));
            // console.log(` --  -- Brick ${supportedBrick.name} is supported by ${supportingBricks.length} bricks`);
            // console.log(supportedBrick, supportingBricks);
            if (supportingBricks.length === 1) {
                // console.log(` --  -- Brick ${brick.name} is the sole support for Brick ${supportedBrick.name}, cannot disintegrate`)
                soleSupport = true;
                break;
            }
        }
        if (!soleSupport) {
            // console.log(` -- Brick ${brick.name} is not the sole support for any brick, can disintegrate`);
            total++;
        }
    }
